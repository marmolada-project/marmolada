...
class AlembicMigration:
    """Glue Marmolada configuration and Alembic DB migrations together."""

    @cached_property
    def config(self) -> alembic.config.Config:
        _config = alembic.config.Config()
        _config.set_main_option("script_location", str(HERE.absolute()).replace("%", "%%"))
        _config.set_main_option(
            "sqlalchemy.url", config["database"]["sqlalchemy"]["url"].replace("%", "%%")
        )
        return _config

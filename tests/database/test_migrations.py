...
from marmolada.database.migrations.main import alembic_migration

HERE = Path(__file__).parent
MIGRATIONSDIR = HERE.parent.parent / "marmolada" / "database" / "migrations"


@pytest.mark.marmolada_config({"database": {"sqlalchemy": {"url": "postgresql://url"}}})
class TestAlembicMigration:
    def test_config(self):
        ...
        assert alembic_migration.config.get_main_option("sqlalchemy.url") == "postgresql://url"
    ...
    def test_config_with_percent_signs(self):
        ...
        with mock.patch("marmolada.database.migrations.main.HERE", MIGRATIONSDIR_WITH_PERCENT):
            assert alembic_migration.config.get_main_option("script_location") == str(
                MIGRATIONSDIR_WITH_PERCENT.absolute()
            )
            assert (
                alembic_migration.config.get_main_option("sqlalchemy.url") == "postgresql://url/%"
            )

    @pytest.mark.parametrize("autogenerate", (False, True))
    @mock.patch("marmolada.database.migrations.main.alembic.command")
    @mock.patch.object(alembic_migration, "_get_current")
    def test_create(self, _get_current, alembic_command, autogenerate, caplog):
        comment = "BOOP"

        with caplog.at_level("DEBUG", "marmolada"):
